...
//                nextActionConfig.getProcessingAction().generateTasks();
    ...
    public static Item archive(Context c, XmlWorkflowItem wfi)
        ...
        notifyOUDissertationCounselor(c, wfi, c.getCurrentUser(), "archive");
    ...
    * @param rejection_message
            ...
            IOException
    {
        ...
        notifyOUDissertationCounselor(c, wi, e, "reject::"+rejection_message);
        ...
        String reason)
    {
        ...
        {
            try {
                EPerson p = wi.getSubmitter();
                // log this email error
                log.warn(LogManager.getHeader(c, "notify_of_reject",
                        "cannot email user" + " eperson_id" + p.getID()
                                + " eperson_email" + p.getEmail()
                                + " workflow_item_id" + wi.getID()));
            } catch (SQLException ex1) {
                java.util.logging.Logger.getLogger(XmlWorkflowManager.class.getName()).log(Level.SEVERE, null, ex1);
            }
        }
    }

    private static void notifyOUDissertationCounselor(Context c, XmlWorkflowItem wi, EPerson e, String message)
    {
        try
        {
            if(null == message || "".endsWith(message)) {
                throw new Exception("The notification message is not specified.");
            }
            ...
            String counselorEmail = e.getEmail();
            if(null == counselorEmail || "".equals(counselorEmail)) {
                counselorEmail = "shareok-dissertation@ou.edu";
            }

            String submitter = getSubmitterInfo(wi);
            ...
            if("archive".equals(message)){
                type = 1;
            }
            else {
                if(!message.contains("::")) {
                    throw new Exception("The notification message is not correctly formated.");
                }
                else {
                    String[] messageInfo = message.split("::");
                    if("reject".equals(messageInfo[0])){
                        type = 0;
                        reason = messageInfo[1];
                    }
                }
            }
            ...
            {
                case 0 :
                    email = Email.getEmail(I18nUtil.getEmailFilename(supportedLocale, "submit_rejector"));
            ...
            //email.addArgument(ConfigurationManager.getProperty("dspace.url") + "/mydspace");
                    break;
                default :
                    throw new Exception("The email notification type is incorrect.");
            }
    ...
    private static String getSubmitterInfo(XmlWorkflowItem wi) throws SQLException {
        ...
        String affiliation = "";
        // Right now we do not put the affiliation information in the letteres:
        //        String group = it.getMetadata("ou.group")[0].value;
        //        if(null != group && !"".equals(group) && group.compareToIgnoreCase("Other") != 0) {
        //            affiliation = group;
        //            if(!group.contains("::")) {
        //                affiliation =  "  " + group;
        //            }
        ...
        //                if(groups.length == 1) {
        //                    affiliation = "  " + groups[0];
        //                }
        //                if(groups.length == 2) {
        //                    affiliation = "  " + groups[1] + ", " + groups[0];
        //                }
        //            }
        //        }
