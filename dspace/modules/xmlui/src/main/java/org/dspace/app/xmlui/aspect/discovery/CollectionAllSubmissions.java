    ...
    public void addBody(Body body) throws SAXException, WingException,
        ...
        if(0 < queryResults.getDspaceObjects().size()){
            ...
            Division lastSubmittedDiv = home
                    .addDivision("collection-all-submission", "secondary all-submission");

            lastSubmittedDiv.setHead(T_head_recent_submissions);

            ReferenceSet allSubmitted = lastSubmittedDiv.addReferenceSet(
                    "collection-all-submitted", null,
                    null, "all-submissions");

            for (DSpaceObject resultObj : queryResults.getDspaceObjects()) {
                if(resultObj != null){
                    allSubmitted.addReference(resultObj);
                }
            }
        }
    }
