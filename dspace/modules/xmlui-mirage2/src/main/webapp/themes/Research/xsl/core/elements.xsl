...
-->
    ...
    <xsl:template match="dri:xref">
				<xsl:if test="not(contains(@target,'author') or contains(@target,'subject') or contains(@target,'degree') or contains(@target, 'dateissued'))">
	        <a>
	            <xsl:if test="@target">
    ...
    <xsl:template match="dri:figure">
		...
		<xsl:template match="dri:div[@interactive='yes']" priority="2">
				<xsl:variable name="pagetype" select="/dri:document/dri:body/dri:div[1]/@n"/>
        <xsl:apply-templates select="@pagination">
            <xsl:with-param name="position">top</xsl:with-param>
        </xsl:apply-templates>
				<xsl:if test="$pagetype!='community-home' and $pagetype!='collection-home'">
						<form>
		            <xsl:call-template name="standardAttributes">
    ...
    <xsl:template match="dri:div/dri:head" priority="3">
				...
				<xsl:if test="not($pagecontainer='hdl:11244/18515' or $pagecontainer='hdl:11244/28096' or $pagecontainer='')">
