...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template name="buildHeader-citizen">
        <header>
            <div class="jumbotron" role="navigation" style="padding: 20px 0">
                <div class="container">
                    <div class="navbar-header">
    ...
    <!--For item Detail View -->
      ...
      <xsl:template name="buildgooglemap-citizensci-search">
          ...
          <xsl:for-each select="/dri:document/dri:body/dri:div/dri:div/dri:list/dri:list/dri:list/dri:list">
              ...
              <xsl:variable name="handleTitle">
                <xsl:choose>
                    <xsl:when test="contains($itemType, 'dc.title')">
                            <xsl:value-of select="."/>
                    </xsl:when>
                    <xsl:otherwise>
                            <xsl:text> </xsl:text>
           ...
           <script><xsl:text>
               ...
               function makemap(points, titles){
                   var map = L.map('citizenscimap-search').setView([35.1879507, -97.4421919], 5);
